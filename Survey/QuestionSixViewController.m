...
@synthesize items;
@synthesize questionSixTableView;
...
- (void)viewDidLoad
{
    ...
    NSString *path = [[NSBundle mainBundle] pathForResource:@"Question6" ofType:@"plist"];
...
{
    //UITableViewCell *tvc = [questionSixTableView cellForRowAtIndexPath:indexPath];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    NSArray *selectedItemsPath = [self.questionSixTableView indexPathsForSelectedRows];
    ...
    for (int i = 0; i < selectedItemsPath.count; i++) {
        ...
        item = [self.questionSixTableView cellForRowAtIndexPath:path].textLabel.text;
