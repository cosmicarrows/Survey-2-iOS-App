...
#import "ResultsFourViewController.h"
#import "AppDelegate.h"

@interface ResultsFourViewController ()

@end

@implementation ResultsFourViewController

@synthesize questionFourTableView = _questionFourTableView;
...
- (void)viewDidLoad
{
    ...
    // Survey answers for question 4 returns an array of strings
    
    // The answers are at index 3
    
    for (int i = 0; i < [[delegate surveys] count]; i++) {
        
        [self.selections addObject:[[[delegate surveys] objectAtIndex:i] objectAtIndex:3]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"Count: %i", count);
    NSLog(@"Total count: %i", totalCount);
